...
checkOutEnhancer.prototype={
		...
		if(errors.size()>0 && errors.size()!=38  &&errors.size()!=43 ){ //38 characters is very strange here because when there is no error, the size of errors is 38... hm..
			...
			if(json.defaultShipping=='off' && json.previousDefaultShipping!=0 ){
				...
				if(json.previousDefaultShipping!=0){
					//alert('-here1.5');
				$('shippingAddress_'+json.previousDefaultShipping).innerHTML=$('shippingAddress_'+json.previousDefaultShipping).innerHTML+'<a id="makeShippingAddress_'+json.previousDefaultShipping+'" class="makeShippingAddressAnchor" href="/account/makedefaultshipping?editAddress='+json.previousDefaultShipping+'"><img src="/htdocs/css/images/ShippingButton.gif" /></a><br />';
	...
	createShippingAddressHtml:function(jsonObject){
		if((jsonObject.defaultShipping=='off' && jsonObject.previousDefaultShipping!=0)||(jsonObject.previousDefaultShipping==0 &&jsonObject.existingAddresses>1 &&jsonObject.defaultShipping=='off')){
			//alert('ba');
		var shippingAddressString = '<div class="shippingAddressBox" id="shippingAddress_'+jsonObject.shippingId+'">Address one: '+jsonObject.address_one+'<br/>'							+   'Address two: '+jsonObject.address_two+'<br/>'
								+	'City: '+jsonObject.city+', '+jsonObject.state+' '+jsonObject.zip+'<br/>'
								+	'Country: '+jsonObject.country+'<br/><a id="deleteShippingAddress_'+jsonObject.shippingId+'" class="deleteShippingAddressAnchor" href="/account/deleteshipping?editAddress='+jsonObject.shippingId+'">Delete</a><br/><a id="makeShippingAddress_'+jsonObject.shippingId+'" class="makeShippingAddressAnchor" href="/account/makedefaultshipping?editAddress='+jsonObject.shippingId+'"><img src="/htdocs/css/images/ShippingButton.gif"></a><br/></div>';
		}else if(jsonObject.defaultShipping=='on' || jsonObject.previousDefaultShipping=='0'){
			//alert('ba2');
			var shippingAddressString = '<div class="shippingAddressBox" id="shippingAddress_'+jsonObject.shippingId+'">Address one: '+jsonObject.address_one+'<br/>'							+   'Address two: '+jsonObject.address_two+'<br/>'
								+	'City: '+jsonObject.city+', '+jsonObject.state+' '+jsonObject.zip+'<br/>'
								+	'Country: '+jsonObject.country+'<br/><a id="deleteShippingAddress_'+jsonObject.shippingId+'" class="deleteShippingAddressAnchor" href="/account/deleteshipping?editAddress='+jsonObject.shippingId+'">Delete</a><br/></div>';
		}
	...
	onMakeShippingSuccess:function(transport){
		...
		if(json.newShippingAddress>0){
			...
			//alert('adding 3');
			$('shippingAddress_'+json.previousShippingAddressId).innerHTML=$('shippingAddress_'+json.previousShippingAddressId).innerHTML+'<a id="makeShippingAddress_'+json.previousShippingAddressId+'" class="makeShippingAddressAnchor" href="/account/makedefaultshipping?editAddress='+json.previousShippingAddressId+'"><img src="/htdocs/css/images/ShippingButton.gif" /></a>';
			//alert('adding 4');
	...
	onCalcPromotionSuccessful:function(transport){
		...
		if(json.successful!=true){
			...
			window.location= 'http://www.dancewearRialto.com/checkout/confirmation';
		}
	}
