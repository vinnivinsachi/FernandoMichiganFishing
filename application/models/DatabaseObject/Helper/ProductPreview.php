<?php
	...
	class DatabaseObject_Helper_ProductPreview extends DatabaseObject
	{
		...
		public static function retrieveUserEmailForProduct($db, $product_id, $product_type, $product_category){
			$select = $db->select();
			if($product_category=='generalSeller'){
			$select->from('user_products', 'User_id')
				->where('products_id = ?', $product_id)
				->where('product_type = ?', $product_type);
			}elseif($product_category=='storeSeller'){
			$select->from('products', 'User_id')
				->where('products_id = ?', $product_id)
				->where('product_type = ?', $product_type);
			}
			...
			$select1=$db->select();
			$select1->from('users', 'email')
				->where('userID = ?', $userId);
			return $db->fetchOne($select1);		
		}
